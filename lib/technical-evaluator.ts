...
1. **摘要深度优化**：
   - **避免结论性重复**：严禁在summary中使用"方案可行/不可行"、"匹配度高"、"表现优秀"等结论性词汇
   - **提供场景洞察**：深入分析为什么该业务场景需要/不需要大模型，传统方案的具体局限性在哪里
   - **给出进阶建议**：当方案整体合理时，必须提供更深层的优化建议（如：建议采用微调、RAG优化、混合架构等）
   - **聚焦核心价值**：说明LLM解决了哪些传统方案无法解决的具体痛点

2. **段落式分析**：每个维度用2-4句连贯的话进行深入分析
3. **平衡视角**：既要看到技术优势，也要识别风险
4. **可操作建议**：提供具体可执行的技术建议
5. **准确评估**：基于用户提供的硬件信息进行客观评估
