...
**评估重点**：
- 技术路线是否清晰成熟？实施难度和时间周期如何？
- 推荐最适合的技术范式（RAG、Fine-tuning、Agent、General等）
- 是否存在不可逾越的技术障碍？
- 提供具体可操作的实施路径建议
...
- **< 50分**：完全没必要使用LLM，是典型的高射炮打蚊子，过度设计

**评估重点**：
- 核心问题是否必须由LLM解决？LLM带来的价值是否超过成本？
- 如果非必要，提供具体的替代方案（如XGBoost模型、BERT分类器、PaddleOCR等）
- 权衡LLM方案与传统方案的投入产出比
...
**评估重点**：
- 模型能力与业务需求是否匹配？模型规模是否适中？模型模态是否正确？
- 模型参数量是否适合当前场景？是否存在过度设计或能力不足？
- 成本效益比是否合理？
...
**评估重点**：
- 数据质量和数量是否充足？是否需要微调？如果需要，数据和模型是否匹配？
- 数据治理是否到位？数据策略是否合理？
- 对于不同技术路线（RAG、微调），评估数据需求的匹配度

### 5. 硬件性能匹配度 (15%)
评估硬件配置能否满足性能需求：
**评分标准**：
- **90-100分**：硬件资源充裕，推理、微调、预训练均可高质量实施
- **70-89分**：硬件资源较好，推理和微调可行，预训练受限
- **50-69分**：硬件资源勉强够用，推理可行，微调和预训练需要优化或硬件升级
- **< 50分**：硬件资源严重不足，仅能满足轻量推理需求

**评估重点**：
- 根据用户实际业务场景（推理优先、微调可选、预训练较少）进行针对性评估
- 如果硬件不足，必须提供具体解决方案：
  - 硬件升级建议（增加GPU数量、升级型号等）
  - 技术优化方案（LoRA、量化、蒸馏、模型并行等）
  - 架构调整建议（模型蒸馏、推理引擎优化等）
- 成本效益是否合理？
