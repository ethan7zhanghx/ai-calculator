...
## 硬件资源评估（已计算）
- **硬件匹配度得分**：${Math.round(resourceFeasibilityScore)} / 100
- **显存占用率**：${resourceFeasibilityScore >= 90 ? '极高(>90%)' : resourceFeasibilityScore >= 70 ? '较高(70-90%)' : resourceFeasibilityScore >= 50 ? '中等(50-70%)' : '较低(<50%)'}
- **评估说明**：以上得分基于精确的显存占用计算得出，请直接采纳为"hardwarePerformanceFit"维度评分。

**重要提醒**：
- 如果得分≥70分：说明硬件资源充足或基本够用，应积极评价硬件配置
- 如果得分50-69分：说明硬件略有不足，建议通过量化等技术手段优化
- 如果得分<50分：说明硬件严重不足，必须明确指出并提供具体的硬件升级建议
