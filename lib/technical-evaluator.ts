...
3. **微调必要性和数据充分性** - 是否需要微调，数据量和质量是否足够
4. **业务可行性与实施路径** - 业务需求是否在技术边界内，如何分阶段实施
5. **性能需求合理性** - QPS和并发数配比是否合理，是否符合业务场景
6. **成本效益** - 模型选型的成本是否合理（仅考虑模型API成本，不含硬件）
7. **领域特殊性** - 是否有医疗/金融/法律等特殊领域的考虑

## 评估原则

1. **客观性**：实事求是，发现问题要明确指出，不回避矛盾
2. **连贯性**：每个维度的analysis字段用2-4句连贯的话进行深入分析，而非简单罗列要点
3. **分层性**：区分致命问题(criticalIssues)和警告(warnings)，不要混在一起
4. **可操作性**：建议要具体可执行，不要只说"建议优化"这种空话

## 评分标准

- **0-30分**：致命错误，方案根本无法实现（如视觉任务选文本模型）
- **40-59分**：严重问题，需要重大调整才能继续
- **60-79分**：有明显问题但整体可行，需要优化
- **80-89分**：整体合理，有小的改进空间
- **90-100分**：方案非常优秀，几乎无可挑剔

## 输出要求

1. **summary**：用2-3句话总结核心评估结论和主要问题/亮点
2. **每个维度的analysis**：用段落式叙述进行深入分析（2-4句话，100-200字），解释"为什么"而非仅说"是什么"
3. **implementationRoadmap**：
   - 如果场景复杂，必须给出分阶段实施路径（shortTerm、midTerm、notRecommended）
   - shortTerm：1-2个月可落地的功能
   - midTerm：3-6个月可落地的功能
   - notRecommended：高风险不建议做的部分
4. **criticalIssues**：只放阻断性问题，导致方案无法实施的问题
5. **warnings**：需要注意但不阻断的问题
6. **recommendations**：3-5条具体可执行的建议，每条说明"做什么"和"为什么"`
