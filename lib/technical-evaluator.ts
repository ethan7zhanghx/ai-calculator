...
**summary 字段是对整个技术方案合理性的核心总结，必须避免重复各维度的结论，而应该提供深层次的技术洞察：**

1. **绝对禁止使用空洞词汇**：
   - ❌ 禁止词汇："表现出色"、"非常适合"、"完美契合"、"充足"、"良好"、"成熟"、"高性价比"、"匹配度高"、"满足需求"等任何结论性、总结性的评价词汇
   - ✅ 必须使用：具体的技术能力描述、对比性分析、可行性解释

2. **必须对比传统方案的局限性**：
   - 明确说明传统方案（如纯OCR、规则引擎、人工处理、传统ML模型）在该场景下的具体技术局限和痛点
   - 用具体的技术术语解释为什么传统方案无法解决问题
   - 例如："传统OCR只能识别文字内容，但无法理解表格结构和字段之间的语义关联"

3. **必须说明LLM的增量价值**：
   - LLM相比传统方案，具体增加了什么技术能力？
   - 解决了哪些过去无法解决的技术难题？
   - 例如："多模态LLM能同时理解图像中的视觉布局和文本内容，自动识别发票的字段类型和数值关系"

4. **必须给出进阶优化建议**：
   - 提供具体可执行的技术优化方向
   - 例如：LoRA微调策略、数据增强方法、RAG架构优化、混合模型部署、模型量化技术等
   - 避免泛泛而谈，要给出具体实施路径

5. **写作风格示例**：
   - ❌ 错误示例："该方案在技术选型上表现出色，PaddleOCR-VL与单据识别场景完美契合，硬件配置充足，方案可行性高"
   - ✅ 正确示例："传统OCR方案只能逐行提取文字，无法理解费用报销单据中'项目名称-金额-日期'的结构化关系，导致提取准确率低。多模态VLM通过视觉语言联合理解，可自动识别表格结构和字段语义，解决了传统方案的核心痛点。建议后续使用企业真实单据数据进行LoRA微调，进一步提升特定格式的识别准确率"
